...
SHA256:vsbccOhzMvf7i8MuiV646jUDdOawpS94Ib0kznOTbxU debian@sshfs-servidor
...
Your public key has been saved in /home/debian/.ssh/id_rsa.pub
The key fingerprint is:
SHA256:g+dxUfCq2BYVpt0GdC2Ix14KuLq2eOCnfg3SW4JYwEY debian@sshfs-cliente
The key's randomart image is:
+---[RSA 3072]----+
|oE     . +*oo.   |
|.o    . o++Bo .  |
...
Formateamos la partición recién creada con un sistema de archivos como `ext4`:
...
Creamos el punto de montaje de la partición y la montamos en el directorio seleccionado:
...
Verificamos que la partición se ha montado correctamente en el directorio que hemos indicado:
...
Vamos a crear un fichero que usaremos como prueba de los posteriores montajes en la máquina **cliente** llamado `prueba.txt`:
...
Instalamos `sshfs` en ambas máquinas a través del siguiente comando:

```
debian@sshfs-servidor:~$ sudo apt install sshfs
```

Necesitamos tener la clave pública de la máquina **cliente** en el fichero `authorized_keys` de la máquina **servidor** para que el montaje funcione correctamente.
...
```
debian@sshfs-cliente:~$ sshfs debian@10.0.0.207:/mount /home/debian/carpeta -o IdentityFile=/home/debian/.ssh/id_rsa
```

Los parámetros que necesitamos son los siguientes:
...
debian@10.0.0.207:/mount  2.0G   28K  1.9G   1% /home/debian/carpeta
```
...
sshfs#debian@10.0.0.207:/mount /mnt/sshfs fuse defaults,_netdev,identityfile=/home/debian/.ssh/id_rsa,allow_other 0 0
```

Esta configuración monta un sistema de archivos remoto SSH en el punto de montaje local `/mnt/sshfs`. 
Utiliza un usuario, una dirección IP y se accede a la carpeta remota utilizando el sistema de archivos `FUSE`; además, se realiza un proceso de identificación mediante una clave privada que se encuentra en `/home/debian/.ssh/id_rsa`.
...
tmpfs                     5.0M     0  5.0M   0% /run/lock
/dev/vda15                124M   12M  113M  10% /boot/efi
debian@10.0.0.207:/mount  2.0G   28K  1.9G   1% /mnt/sshfs
tmpfs                      97M     0   97M   0% /run/user/1000
```
...
/dev/vda15                124M   12M  113M  10% /boot/efi
tmpfs                      97M     0   97M   0% /run/user/1000
debian@10.0.0.207:/mount  2.0G   28K  1.9G   1% /home/debian/sshfs
```
